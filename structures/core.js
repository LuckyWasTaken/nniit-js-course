...
//capitalizeAllWords('vvv bbb aaa kk');
...
function multiply(obj) {
  for (var key in obj){
    if (typeof(obj[key])==='number') { //////I write 'typeof(7)', becouse in test use int.
      obj[key]=obj[key]*2;
    }
  }
...
//console.log(isPali('А, шорохами зима хороша'));
//Напишите функцию, определяющую, является ли данное слово палиндромом
function isPali(str) {
  
  const buf=str.split(''); //for use filter()
  const buf2=buf.filter((x) => x!=' ' && x!='.' && x!=',' && x!='!' && x!='?' && x!='-').map(x=>x.toLowerCase());
