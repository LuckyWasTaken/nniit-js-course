...
router.get("/test", [], async (req, res) => {
  console.log("/test");
  return res.status(300).json({ asda: "AAAA" });
});
...
router.post(
  ...
  [
    ...
    check(
      "password",
      "Password should be not shorter than 6 symbols"
    ).isLength({ min: 6 }),
  ],
  async (req, res) => {
    try {
      ...
      if (candidate) {
        return res
          .status(400)
          .json({ message: "Sorry, such user is alredy exist!" });
      }
...
router.post(
  "/login",
  [
    check("email", "Please, enter correct password.")
      .normalizeEmail()
      .isEmail(),
